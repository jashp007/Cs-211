...
void initializeGame(struct GameInfo *gameInfo)   // All game settings variables
{
    ...
    gameInfo->pit1 = randomNumbers[0];   
    gameInfo->pit2 = randomNumbers[1];
    gameInfo->wumpusRoom = randomNumbers[2];
    gameInfo->playerRoom = randomNumbers[3];
...
void shootArrow(struct GameInfo gameInfo, int **Rooms){
...
{
    printf("Enter the room locations (1..20) for player, wumpus, pit1, pit2, bat1, bat2: \n");
...
scanf(" %d %d %d %d %d %d", &theGameInfo->playerRoom, &theGameInfo->wumpusRoom, 
                          &theGameInfo->pit1, &theGameInfo->pit2, &theGameInfo->bat1, &theGameInfo->bat2);
    printf("\n");
}
...
printf("%d. Enter your move (or 'D' for directions): ", gameInfo.moveNum);
...
gameInfo.moveNum = gameInfo.moveNum + 1;
