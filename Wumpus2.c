//When i started reading the program description i was confused and then i went through a lot of piazza posts and tried to understand by myself and i also had a lot if hws that week and on top iof that i was sick so when i was feeling better deadline was nearby and i got anxious and started worrying about the program and when i knew what i have to do it looked a bit tricky a starting and i think it was when i reached the one of the most difficult part of implementation which was shoot arrow function. When I started writing the program I was able to finish almost the basic functions which were kind of similar to the previous program ones and then when I reached the shoot arrow part I faced a lot of trouble and especially when the arrow rechociates. And then I read a couple of piazza posts and tried to understand the thing but had a hard time understanding it. And then finally when i got everything i started writing the code and when it was a time of deadline i was left with some work and i tried to finish it asap. And somehow i finished it and uploaded and got a couple of test cases failing that were 8,10,13,14,16,17. So I started solving test cases and for the 8th one my code showed some other wumpus room and the output had some other so I went through all the code and changed the shoot arrow function. For case 12 it looked like a print statement error which was printing the correct line but in a different location. And for the rest of cases 2 were hidden and others were having a problem like the above ones. Talking about similarities of solution and my code is that a lot of functions are same for almost everyone. Like the print ones and the other which were used for checking things like adjacent rooms and hazards and all. Because the algorithm is kinda the same for everyone so they might be the same for me and maybe a lot of students. But the difference is that the professor wrote a function which checked the conditions for bats which worked in the way that it checks the room if it has bats and they used various do while loops to check the conditions if the room player enters has bats in it or not. While on the other hand I handled those conditions in various functions itself. That kinda made my code a bit shorter but i don’t know whose code is efficient in case of big O complexity and all things. Another thing in the shoot arrow function which troubled me a lot. I used a couple of nested if else and used while loops which made some complex things easier for me. The solution uploaded had malloc in the function of shooting an arrow on the other hand i handled that condition without using malloc. 
